...
public partial class CheckIfSerializableTests
{
    ...
    [Theory]
    [InlineData("X, Y", "T1, T2, T3", "5, 10, 3", "E_1-r1(X) r2(Y) w2(Y) r3(Y) w1(X) c1", "E_1-ROLLBACK-4")]
    [InlineData("X, Y, Z", "T1, T2, T3", "5, 10, 3", "E_2-w2(X) r1(Y) w3(X) r2(Z) w1(Z) c1", "E_2-ROLLBACK-3")]
    [InlineData("X, Y, Z", "T1, T2, T3", "5, 10, 3", "E_3-r3(X) w3(Y) c1 r1(X) w1(Y) c2 r2(Y) w2(Z) c3", "E_3-OK")]
    public async Task CheckIfSerializableAsync_WithProvidedSchedules_ShouldReturnExpectedResult(
        string dataRecordsString, string transactionsString, string timestampsString,
        string scheduleString, string expected)
    {
        // Arrange
    ...
    {
        return dataRecordsString.Split(',')
